...
import { Trash2, Settings, Save, Key, UserPlus, Edit, Users, Shield, ToggleLeft, X, Lock, Download, Upload, Database, MessageCircle, Palette, Image } from "lucide-react";
...
export default function Admin() {
  ...
  const [adminSettings, setAdminSettings] = useState<AdminSettings>({
    ...
    requisitionHeaderTitle: "Daily Our Bangladesh",
    expenseHeaderTitle: "Daily Our Bangladesh",
    requisitionHeaderSubtitle: "Requisition Month of",
    expenseHeaderSubtitle: "Expense Month of",
    requisitionPreparedBy: "Administrator",
    expensePreparedBy: "Administrator",
  ...
  useEffect(() => {
    ...
    if (storedFeatures) {
      ...
      setAssignmentEnabled(features.assignmentEnabled ?? true);
    }
  ...
  const handleSaveEditModerator = (id: string) => {
    ...
    toast({
      ...
      description: "Login credentials have been updated",
    });
  };
  ...
  const handleDeleteModerator = (id: string, name: string) => {
    ...
    toast({
      ...
      description: `${name} has been removed from credentials`,
    });
  };
  ...
  const handleToggleFeature = (feature: "voiceArtist" | "attendance" | "workFlow" | "videoUploadTime" | "assignment", enabled: boolean) => {
    const storedFeatures = localStorage.getItem("dob_feature_toggles");
    ...
    const features = {
      ...
      assignmentEnabled: feature === "assignment" ? enabled : (currentFeatures.assignmentEnabled ?? assignmentEnabled),
    };
    ...
    } else if (feature === "videoUploadTime") {
    ...
    const featureName = 
      ...
      feature === "videoUploadTime" ? "Video Upload Time" :
      "Assignment";
  ...
  const handleFileUpload = (file: File, type: 'logo' | 'favicon' | 'loginLogo' | 'loginBackground') => {
    const reader = new FileReader();
    reader.onload = (e) => {
      ...
      if (type === 'logo') {
        ...
        updateCustomization({ faviconUrl: result });
      } else if (type === 'loginLogo') {
        updateCustomization({ loginPage: { ...customization.loginPage, logoUrl: result } });
      } else if (type === 'loginBackground') {
        updateCustomization({ loginPage: { ...customization.loginPage, backgroundImageUrl: result } });
      }
      toast({
        title: "File Uploaded",
              ...
              <Card className="border-green-200 dark:border-green-900">
                <CardHeader>
                  <div className="flex items-center gap-2">
                    ...
                    <CardTitle className="text-green-700 dark:text-green-400">Page Header Settings</CardTitle>
                  </div>
                  <CardDescription>Customize headers for Requisition and Expense pages</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="space-y-4">
                    <div className="space-y-2">
                      <Label htmlFor="requisitionHeaderTitle">Requisition Header Title</Label>
                      <Input
                        id="requisitionHeaderTitle"
                        value={adminSettings.requisitionHeaderTitle || "Daily Our Bangladesh"}
                        onChange={(e) => setAdminSettings({ ...adminSettings, requisitionHeaderTitle: e.target.value })}
                        placeholder="Daily Our Bangladesh"
                      />
                    </div>

                    <div className="space-y-2">
                      <Label htmlFor="requisitionHeaderSubtitle">Requisition Header Subtitle</Label>
                      <Input
                        id="requisitionHeaderSubtitle"
                        value={adminSettings.requisitionHeaderSubtitle || "Requisition Month of"}
                        onChange={(e) => setAdminSettings({ ...adminSettings, requisitionHeaderSubtitle: e.target.value })}
                        placeholder="Requisition Month of"
                      />
                    </div>

                    <div className="space-y-2">
                      <Label htmlFor="requisitionPreparedBy">Requisition Prepared By</Label>
                      <Input
                        id="requisitionPreparedBy"
                        value={adminSettings.requisitionPreparedBy || "Administrator"}
                        onChange={(e) => setAdminSettings({ ...adminSettings, requisitionPreparedBy: e.target.value })}
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="expenseHeaderTitle">Expense Header Title</Label>
                      <Input
                        id="expenseHeaderTitle"
                        value={adminSettings.expenseHeaderTitle || "Daily Our Bangladesh"}
                        onChange={(e) => setAdminSettings({ ...adminSettings, expenseHeaderTitle: e.target.value })}
                        placeholder="Daily Our Bangladesh"
                      />
                    </div>

                    <div className="space-y-2">
                      <Label htmlFor="expenseHeaderSubtitle">Expense Header Subtitle</Label>
                      <Input
                        id="expenseHeaderSubtitle"
                        value={adminSettings.expenseHeaderSubtitle || "Expense Month of"}
                        onChange={(e) => setAdminSettings({ ...adminSettings, expenseHeaderSubtitle: e.target.value })}
                        placeholder="Expense Month of"
                      />
                    </div>

                    <div className="space-y-2">
                      <Label htmlFor="expensePreparedBy">Expense Prepared By</Label>
                      <Input
                        id="expensePreparedBy"
                        value={adminSettings.expensePreparedBy || "Administrator"}
                        onChange={(e) => setAdminSettings({ ...adminSettings, expensePreparedBy: e.target.value })}
                        placeholder="Administrator"
                      />
                    </div>
                  </div>
              ...
              <Card className="border-orange-200 dark:border-orange-900">
                <CardHeader>
                  <div className="flex items-center gap-2">
                    ...
                    <CardTitle className="text-orange-700 dark:text-orange-400">Attendance Settings</CardTitle>
                  </div>
                  <CardDescription>Configure office timing and attendance rules</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      id="officeStartTime"
                      type="time"
                      value={adminSettings.officeStartTime || "08:00"}
                      onChange={(e) => setAdminSettings({ ...adminSettings, officeStartTime: e.target.value })}
                      className="h-11"
                    />
                    <p className="text-xs text-muted-foreground">
                      Employees clocking in after this time will be automatically marked as Late
                    </p>
                  </div>
              ...
              <Card className="border-blue-200 dark:border-blue-900">
                <CardHeader>
                  <div className="flex items-center gap-2">
                    ...
                    <CardTitle className="text-blue-700 dark:text-blue-400">App Customization</CardTitle>
                  </div>
                  <CardDescription>Customize app branding, theme colors, and login page</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => updateCustomization({ appName: e.target.value })}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-3">
                      {customization.logoUrl && (
                        <div className="w-12 h-12 rounded border flex items-center justify-center overflow-hidden">
                          <img src={customization.logoUrl} alt="Logo" className="max-w-full max-h-full object-contain" />
                        </div>
                      )}
                      <Input
                        type="file"
                    ...
                    <p className="text-xs text-muted-foreground">Upload app logo (displayed in header)</p>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    />
                    <p className="text-xs text-muted-foreground">Upload favicon (browser tab icon)</p>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <div className="flex gap-2">
                          <Input
                            id="primaryColor"
                            type="color"
                            value={customization.theme.primary}
                            onChange={(e) => updateCustomization({ theme: { ...customization.theme, primary: e.target.value } })}
                      ...
                      <div>
                        ...
                        <div className="flex gap-2">
                          <Input
                            id="secondaryColor"
                            type="color"
                            value={customization.theme.secondary}
                            onChange={(e) => updateCustomization({ theme: { ...customization.theme, secondary: e.target.value } })}
                            className="h-10 w-16 p-1"
                          />
                          <Input
                            type="text"
                      ...
                      <div>
                        ...
                        <div className="flex gap-2">
                          <Input
                            ...
                            onChange={(e) => updateCustomization({ theme: { ...customization.theme, background: e.target.value } })}
                            className="h-10 w-16 p-1"
                          />
                          <Input
                            type="text"
                      ...
                      <div>
                        <Label htmlFor="foregroundColor" className="text-xs">Text</Label>
                        <div className="flex gap-2">
                          <Input
                            id="foregroundColor"
                            type="color"
                            value={customization.theme.foreground}
                            onChange={(e) => updateCustomization({ theme: { ...customization.theme, foreground: e.target.value } })}
                            className="h-10 w-16 p-1"
                          />
                          <Input
                            type="text"
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-3">
                        {customization.loginPage.logoUrl && (
                          <div className="w-12 h-12 rounded border flex items-center justify-center overflow-hidden">
                            <img src={customization.loginPage.logoUrl} alt="Login Logo" className="max-w-full max-h-full object-contain" />
                          </div>
                        )}
                        <Input
                          type="file"
                          ...
                          onChange={(e) => {
                            const file = e.target.files?.[0];
                            if (file) handleFileUpload(file, 'loginLogo');
                          }}
                        />
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          const file = e.target.files?.[0];
                          if (file) handleFileUpload(file, 'loginBackground');
                        }}
                      />
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        id="welcomeText"
                        value={customization.loginPage.welcomeText}
                        onChange={(e) => updateCustomization({ loginPage: { ...customization.loginPage, welcomeText: e.target.value } })}
                    ...
                    <div className="flex items-center justify-between">
                      <div className="space-y-0.5">
                        <Label>Show Developer Credit</Label>
                        <p className="text-xs text-muted-foreground">Display developer credit on login page</p>
                      </div>
                      <Switch
                        checked={customization.loginPage.showDeveloperCredit}
                        onCheckedChange={(checked) => updateCustomization({ loginPage: { ...customization.loginPage, showDeveloperCredit: checked } })}
                      />
                    </div>
                  </div>
              ...
              <Card className="border-purple-200 dark:border-purple-900">
                ...
                <CardContent className="space-y-3">
                  {[
                    ...
                    { id: "assignment", label: "Assignment Module", desc: "Create and manage assignments with priorities and deadlines", enabled: assignmentEnabled },
                  ].map((feature) => (
              ...
              <Card className="border-blue-200 dark:border-blue-900">
                <CardHeader>
                  <div className="flex items-center gap-2">
                    ...
                    <CardTitle className="text-blue-700 dark:text-blue-400">Employee Login Credentials</CardTitle>
                  </div>
                  <CardDescription>Manage employee login credentials and access</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="p-3 bg-blue-50 dark:bg-blue-950/20 rounded-lg border border-blue-200 dark:border-blue-900 text-xs text-blue-800 dark:text-blue-400 space-y-1">
                    <p className="font-medium">💡 Note:</p>
                    <p>Employee credentials are automatically created when you add/update employees in the Employee Data section. You can also manually add additional login credentials below.</p>
                  </div>

                  <div className="p-3 bg-muted/50 rounded-lg space-y-2.5">
                    <h3 className="font-semibold text-sm flex items-center gap-2">
                      <UserPlus className="w-4 h-4" />
                      Add Manual Login Credential
                    </h3>
                    <div className="space-y-2">
                      <Input
                  ...
                  <div className="space-y-2">
                    <h3 className="font-semibold text-sm">All Login Credentials ({moderators.length})</h3>
                    {moderators.length === 0 ? (
                      <p className="text-sm text-muted-foreground text-center py-6">
                        No credentials yet. Create employees in the Employee Data section or add one above to get started.
                      </p>
                    ) : (
