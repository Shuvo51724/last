...
import { getEmployeeUserByUserId } from "@/lib/employeeUserService";
import { recordCheckIn, recordCheckOut } from "@/lib/attendanceService";

export type UserRole = "admin" | "moderator" | "superModerator" | "employee";
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const login = async (userId: string, password: string): Promise<{ success: boolean; message?: string }> => {
    ...
    const employeeUser = getEmployeeUserByUserId(userId);
    if (employeeUser && employeeUser.password === password) {
      ...
      const employee = employees.find((e: any) => e.employeeId === employeeUser.employeeId);
      ...
      const storedSettings = localStorage.getItem("dob_settings");
      const officeStartTime = storedSettings 
        ? JSON.parse(storedSettings).officeStartTime || "08:00"
        : "08:00";
